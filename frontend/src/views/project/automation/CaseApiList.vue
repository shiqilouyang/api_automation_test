    ...
    <section>
        ...
        <el-table :data="ApiList" highlight-current-row v-loading="listLoading" style="width: 100%;">
            ...
            <el-table-column label="操作" min-width="20%">
                <template slot-scope="scope">
                    ...
                    <el-button type="primary" size="small" @click="Test1(scope.$index, scope.row)">压测</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-col :span="24" class="toolbar">
        ...
        <el-dialog title="测试结果" :visible.sync="TestResult" :close-on-click-modal="false">
            <div style="height:700px;overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6">
                ...
                <div v-if="result.responseData&&flag" class="resultStyle" style="overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6;padding: 10px;width: 90%;word-break: break-all;line-height:25px"><pre v-highlightA><code>{{result.responseData}}</code></pre></div>
...
<script>
    import { test,pretest } from '../../../api/api'
    import $ from 'jquery'
        ...
        methods: {
            ...
            Test1(index, row) {
                if (this.url) {
                    ...
                    $.ajax({
                        type: "post",
                        url: test+"/api/automation/pressure_start_test",
                        async: true,
                        data: JSON.stringify({
                            project_id: Number(this.$route.params.project_id),
                            case_id: Number(this.$route.params.case_id),
